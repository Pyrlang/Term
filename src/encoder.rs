...
impl<'a> Encoder<'a> {
  ...
  pub fn encode_default(&mut self, term: &PyObject) -> CodecResult<()> {
    ...
    match type_name_ref {
      ...
      "ImproperList" => {
        ...
        self.write_list_no_tail(&elements).unwrap();
  ...
  fn write_list_no_tail(&mut self, list: &PyList) -> CodecResult<()> {
    ...
    self.data.write_u32::<BigEndian>(size as u32).unwrap();
  ...
  fn write_arbitrary_int(&mut self, val: &PyObject, size: u32) -> CodecResult<()> {
    if size < 256 {
      ...
      self.data.write_u32::<BigEndian>(size).unwrap();
    }
  ...
  fn write_4byte_int(&mut self, val: i64) -> CodecResult<()> {
    if val >= 0 && val <= u8::MAX as i64 {
      ...
      self.data.write_i32::<BigEndian>(val as i32).unwrap();
    } else {
  ...
  fn write_float(&mut self, val: f64) -> CodecResult<()> {
    ...
    self.data.write_f64::<BigEndian>(val).unwrap();
    Ok(())
  }
  ...
  fn write_atom_from_cow(&mut self, text: Cow<str>) -> CodecResult<()> {
    ...
    if str_byte_length <= u8::MAX as usize {
      ...
      self.data.write(byte_array).unwrap(); // write &[u8] string content
    } else if str_byte_length <= u16::MAX as usize {
      self.data.push(consts::TAG_ATOM_UTF8_EXT);
      self.data.write_u16::<BigEndian>(str_byte_length as u16).unwrap(); // 16bit length
      self.data.write(byte_array).unwrap(); // write &[u8] string content
    } else {
  ...
  fn write_str(&mut self, py_str: &PyString) -> CodecResult<()> {
    ...
    if str_byte_length <= u8::MAX as usize && can_be_encoded_as_bytes {
      ...
      self.data.write_u32::<BigEndian>(chars_count as u32).unwrap(); // chars, not bytes!
  ...
  fn write_pid(&mut self, py_pid: &PyObject) -> CodecResult<()> {
    ...
    self.write_atom_from_cow(node_name.to_string(self.py)?).unwrap();
    self.data.write_u32::<BigEndian>(id).unwrap();
    self.data.write_u32::<BigEndian>(serial).unwrap();
    self.data.write_u32::<BigEndian>(creation).unwrap();
  ...
  fn write_ref(&mut self, py_ref: &PyObject) -> CodecResult<()> {
    ...
    self.data.write_u16::<BigEndian>((id.len() / 4) as u16).unwrap();
    self.write_atom_from_cow(node_name.to_string(self.py)?).unwrap();
    self.data.write_u32::<BigEndian>(creation).unwrap();
    self.data.write(id).unwrap();
  ...
  fn write_binary(&mut self, py_bytes: &PyBytes) -> CodecResult<()> {
    ...
    self.data.write_u32::<BigEndian>(data.len() as u32).unwrap();
    self.data.write(data).unwrap();
    Ok(())
  }
